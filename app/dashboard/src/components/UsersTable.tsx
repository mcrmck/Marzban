...
export const UsersTable: FC<UsersTableProps> = (props) => {
  ...
  useEffect(() => {
    // Create a logging container if it doesn't exist
    let logContainer = document.getElementById('custom-log-container');
    if (!logContainer) {
      logContainer = document.createElement('div');
      logContainer.id = 'custom-log-container';
    ...
    const customLog = (message: string, type: 'info' | 'error' | 'warn' = 'info') => {
      const timestamp = new Date().toISOString().split('T')[1].split('.')[0];
      const logEntry = document.createElement('div');
      logEntry.style.cssText = `
        margin-bottom: 5px;
        padding: 3px;
        border-left: 3px solid ${type === 'error' ? 'red' : type === 'warn' ? 'yellow' : 'green'};
      `;
    ...
    customLog('TEST: Basic info log');
    customLog('TEST: Error log', 'error');
    customLog('TEST: Warning log', 'warn');

    // Test with side effects
    (() => {
      customLog('TEST: IIFE log');
    })();

    // Test with window assignment
    if (typeof window !== 'undefined') {
      (window as any).debugTest = "Custom log test executed";
      customLog('TEST: Window assignment log');
    }

    // Cleanup function
    return () => {
      const container = document.getElementById('custom-log-container');
      if (container) {
        container.remove();
      }
    };
